...
export default function RepoNode({ data, selected }: NodeProps) {
  ...
  if (zoom < 0.35) {
    return (
      <div className={`bg-slate-800 border-2 rounded-lg px-2 py-1.5 flex items-center gap-1.5 ${
        ...
        <Handle type="target" position={Position.Top} className="!bg-blue-400 !border-blue-600 !w-2 !h-2" />
        <div className="w-2 h-2 rounded-full bg-blue-400 flex-shrink-0" />
        <span className="text-white text-xs font-semibold truncate">{d.label}</span>
        <Handle type="source" position={Position.Bottom} className="!bg-blue-400 !border-blue-600 !w-2 !h-2" />
      </div>
    )
  }
  ...
  return (
    <div className={`bg-slate-800 border-2 rounded-xl px-4 py-3 shadow-lg ${
      ...
      <Handle type="target" position={Position.Top} className="!bg-blue-400 !border-blue-600 !w-3 !h-3" />
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-blue-400 !border-blue-600 !w-3 !h-3" />
    </div>
  )
}
